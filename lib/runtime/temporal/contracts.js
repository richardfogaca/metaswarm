...
const WORK_UNIT_ACTION_KINDS = new Set([
  'implement',
  'validate',
  'adversarial_review',
  'commit',
...
function validateWorkUnitAction(action, fieldName = 'workflowState.workUnitAction') {
  assertRecord(action, fieldName);
  assertEnum(action.kind, WORK_UNIT_ACTION_KINDS, `${fieldName}.kind`);
  assertSafePathToken(action.workUnitId, `${fieldName}.workUnitId`);
  assertSafePathToken(action.actionKey, `${fieldName}.actionKey`);
  assertSafePathToken(action.artifactKey, `${fieldName}.artifactKey`);

  if (action.kind !== 'implement') {
    assertSafePathToken(action.sourceArtifactKey, `${fieldName}.sourceArtifactKey`);
  } else if (action.sourceArtifactKey !== undefined) {
    assertSafePathToken(action.sourceArtifactKey, `${fieldName}.sourceArtifactKey`);
  }
...
function validateRuntimeWorkflowState(workflowState) {
  ...
  if (workflowState.kind === 'run_work_unit_action') {
    if (workflowState.workUnitAction === undefined) {
      throw new TypeError('workflowState.workUnitAction is required for run_work_unit_action');
    }
  }
  ...
  if (workflowState.workUnitAction !== undefined) {
    validateWorkUnitAction(workflowState.workUnitAction);
  }
  ...
  return {
    ...
    specToPlanAction:
      workflowState.specToPlanAction === undefined
        ...
        : validateSpecToPlanAction(workflowState.specToPlanAction),
    workUnitAction:
      workflowState.workUnitAction === undefined
        ? undefined
        : validateWorkUnitAction(workflowState.workUnitAction),
...
function validateStep7WorkflowState(workflowState) {
  return validateRuntimeWorkflowState(workflowState);
}
...
function buildStep4ReviewArtifact({ input, workflowState, endedAt }) {
  ...
  if (normalizedState.kind === 'complete') {
    ...
    humanActionRequired = null;
  } else if (normalizedState.kind === 'run_work_unit_action') {
    blockers = blockers ?? ['Running work-unit action'];
    humanActionRequired = null;
  }
