...
const { loadBeadsWorkflowState } = require('../beads-workflow-state');
const { executeIdempotentLateStageAction } = require('../late-stage-actions');
const { executeIdempotentSpecToPlanAction } = require('../spec-to-plan-actions');
const { executeIdempotentWorkUnitAction } = require('../work-unit-actions');
...
function createTemporalActivities({
  ...
  return {
    ...
    async readWorkflowState(input) {
      return loadBeadsWorkflowState({
        repoRoot,
        beadsId: input.beadsId,
      });
    },
    async readBeadsWorkflowState({ beadsId }) {
      return loadBeadsWorkflowState({
        repoRoot,
        beadsId,
      });
