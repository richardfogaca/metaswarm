...
const RUNTIME_START_MODES = new Set(['immediate', 'delayed_once', 'recurring']);
...
function normalizeCreateRequest(createRequest) {
  ...
  if (createRequest.externalRef !== undefined) {
    assertNonEmptyString(
      createRequest.externalRef,
      'launchRecord.materialization.createRequest.externalRef'
    );
  }
...
function validateLaunchRecord(launchRecord) {
  ...
  if (launchRecord.runtimeStart !== undefined) {
    ...
    } else if (launchRecord.runtimeStart.mode === 'recurring') {
      assertIsoTimestamp(launchRecord.runtimeStart.scheduledFor, 'launchRecord.runtimeStart.scheduledFor');
      assertNonEmptyString(
        launchRecord.runtimeStart.occurrenceKey,
        'launchRecord.runtimeStart.occurrenceKey'
      );
    } else if (launchRecord.runtimeStart.scheduledFor !== undefined) {
      throw new TypeError('launchRecord.runtimeStart.scheduledFor is only valid for delayed_once or recurring');
    }

    if (
      launchRecord.runtimeStart.mode !== 'recurring' &&
      launchRecord.runtimeStart.occurrenceKey !== undefined
    ) {
      throw new TypeError('launchRecord.runtimeStart.occurrenceKey is only valid for recurring');
    }
  }
...
function deriveDateParts(timestamp, timezone) {
...
function buildTemplateContext({ runId, taskDefinitionId, initiatedAt, scheduledFor, timezone }) {
  const dateParts = deriveDateParts(scheduledFor ?? initiatedAt, timezone);
...
function buildCreateBeadsRequest({
  ...
  return normalizeCreateRequest({
    ...
    priority: materialization.priority,
    parentBeadsId: materialization.parentBeadsId,
    externalRef: `metaswarm:run:${runId}`,
  });
}
