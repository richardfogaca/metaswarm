...
function validateLaunchRecord(launchRecord) {
  ...
  if (launchRecord.triggerType !== 'ad_hoc') {
    throw new TypeError('launchRecord.triggerType must be ad_hoc');
  }
  assertIsoTimestamp(launchRecord.initiatedAt, 'launchRecord.initiatedAt');
  if (launchRecord.initiatedBy !== 'operator') {
    throw new TypeError('launchRecord.initiatedBy must be operator');
  }
  ...
  if (workflowInput.triggerType !== 'ad_hoc') {
    throw new TypeError('launchRecord.workflowInput.triggerType must be ad_hoc');
  }
  if (workflowInput.initiatedBy !== 'operator') {
    throw new TypeError('launchRecord.workflowInput.initiatedBy must be operator');
  }
